...
- Low Trust 旧“三段警告各按一次”和单一 18 窗玩法均已退役，当前改为三轮独立警告。
- 第一轮蓝色：8 个、初始 3 个、间隔 `0.50s`；第二轮橙色：11 个、初始 4 个、间隔 `0.34s`；第三轮红色：14 个、初始 5 个、间隔 `0.20s`。
- 每轮先关闭居中的主警告，再清空本轮持续生成的浮窗；三轮完成后才关机。High Trust 仍只需一个授权窗口。
